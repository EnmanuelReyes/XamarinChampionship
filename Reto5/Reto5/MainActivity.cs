...
namespace Reto5
{
    ...
    public class MainActivity : Activity
    {
        ...
        const string emailParticipante = "enmanuelreyes@live.com";
        const string pais = "do"; // Consulta el código de país en http://www.worldstandards.eu/other/tlds/
        Button btnRegistro;
        ...
        private async Task RefreshItemsFromTableAsync()
        {
            try
            {
                ...
                string textToDisplay = ((TextView)FindViewById(Resource.Id.textViewResults)).Text + "\n\n";
                for (int i = 0; i < resultado.Count; i++)
                {
                    textToDisplay = textToDisplay + resultado[i].Email + " " + resultado[i].Reto + "\n";
                }

                ((TextView)FindViewById(Resource.Id.textViewResults)).Text = textToDisplay;
            }
            catch (Exception e)
            {
                string error = e.Message;
